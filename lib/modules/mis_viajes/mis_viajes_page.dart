          ...
          children: [
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                  onPressed: () =>
                      _selectDate(context, true), // Para fecha de inicio
                  child: Text('Seleccionar fecha de inicio'),
                ),
                ElevatedButton(
                  onPressed: () =>
                      _selectDate(context, false), // Para fecha de finalización
                  child: Text('Seleccionar fecha de finalización'),
                ),
              ],
            ),
            SizedBox(height: akContentPadding),
  ...
  Future<void> _selectDate(BuildContext context, bool isStartDate) async {
    final DateTime? picked = await showDatePicker(
      ...
      initialDate: isStartDate
          ? _conX.selectedStartDate ?? DateTime.now()
          : _conX.selectedEndDate ?? DateTime.now(),
      firstDate: DateTime(2000),
      lastDate: DateTime(2025),
    );
    if (picked != null) {
      if (isStartDate) {
        _conX.selectedStartDate = picked;
      } else {
        _conX.selectedEndDate = picked;
      }
      _conX.filterListByDate();
    }
  }
}
